...
app.controller('TabServiceCtrl', ['$scope','$http', '$ionicSlideBoxDelegate', '$timeout,AuthService', '$ionicHistory','$stateParams',function($scope,$http, $ionicSlideBoxDelegate, $timeout,AuthService, $ionicHistory,$stateParams) {
...
app.controller('clearservice',['$http','$scope', '$state','$stateParams','$timeout',function($http,$scope, $state,$stateParams,$timeout){
// money
	...
	var getList = function () {
		$http.get(SETTING.ApiUrl + '/Service/Get', {
...
//		$("#li"+sel:even).attr("class","distance border-css specialStyle");
	...
	$scope.buysome=function(){
		if($scope.selected!=undefined && $scope.selected!="") {
			...
			//});
			$state.go("page.order",{productId:$scope.selected,count:1,type:'service'})
		}
	}
}]);

app.controller('safeservice',['$http','$scope','$state','$stateParams','$timeout',function($http,$scope,$state,$stateParams,$timeout){
// no money
